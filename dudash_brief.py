...
Write today's script and bullets now."""
    ...
    # Diagnostic info in case the response is empty or oddly short --
    # printed unconditionally but cheap, and invaluable when the word
    # count check downstream fails and we need to know why.
    stop_reason = data.get("stop_reason", "unknown")
    content_blocks = data.get("content", [])
    block_types = [b.get("type") for b in content_blocks]
    print(f"API stop_reason: {stop_reason}; content block types: {block_types}")
    ...
    if not full_text:
        # Nothing usable came back at all. Print the raw response (minus
        # nothing sensitive -- there's no user data here, just the model's
        # own output) so the next run's log actually explains why, instead
        # of just reporting "0 words" with no further clue.
        print(f"WARNING: model returned no text content. Full API response: {data}")
...
def main() -> None:
    ...
    headline_count = len([l for l in headlines.splitlines() if l.strip()])
    print(f"Fetched {headline_count} headline lines.")
    if headline_count < 5:
        print("WARNING: very few headlines retrieved — feeds may be down or blocking this request.")
    ...
    if word_count < 300:
        raise RuntimeError(
            f"Script came back only {word_count} words (expected roughly "
            "1000+). Refusing to generate audio or send an email from a "
            "script this short — it's almost certainly not a real brief. "
            "Check the raw model output and headline/FRED input above."
        )
